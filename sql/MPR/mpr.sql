...
Initial_COGS as (
	select
		...
		case    when  (MPR.Vertical not in ('HA') and MPR.PaymentTypeGroup in ('Card','AmEx-Processing'))  then
				...
				when (MPR.Vertical in ('HA') and MPR.PaymentTypeGroup in ('AmEx-Processing')) then 
					coalesce( ((Credit_Card_Net_USD - Amex_Processing_Net_USD) * Costs.Credit * 0.01),0) + coalesce((Debit_Card_Net_USD * Costs.Debit * 0.01),0) + coalesce((Amex_Processing_Net_USD * Costs.Amex * 0.01),0) + coalesce((case when TPV is null and PaymentTypeGroup in ('Card') then TPV_Billing else 0 end * Costs.Blend * 0.01),0)
				when  MPR.Vertical in ('HA') and MPR.PaymentTypeGroup in ('Card') and FeePaymentType in ('PropertyPaid') then
	...
	where MPR.Gateway in ('YapProcessing') and MPR.Vertical not in ('Intl') and MPR.PaymentTypeGroup not in ('Cash')
	group by
		MPR.Date
),
COGS_Financials as (
...
MPR as (
	select
		...
		case    when  (MPR.Vertical not in ('HA') and MPR.PaymentTypeGroup in ('Card','AmEx-Processing')) then
				...
				when (MPR.Vertical in ('HA') and MPR.PaymentTypeGroup in ('AmEx-Processing')) then 
					coalesce( ((Credit_Card_Net_USD - Amex_Processing_Net_USD) * Costs.Credit * 0.01),0) + coalesce((Debit_Card_Net_USD * Costs.Debit * 0.01),0) + coalesce((Amex_Processing_Net_USD * Costs.Amex * 0.01),0) + coalesce((case when TPV is null and PaymentTypeGroup in ('Card') then TPV_Billing else 0 end * Costs.Blend * 0.01),0)
				when MPR.Vertical in ('HA') and MPR.PaymentTypeGroup in ('Card') and MPR.FeePaymentType in ('PropertyPaid') then	
